...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            new_pos = ghost_up(ghost_pos, self.n, self.walls)
        if new_pos != ghost_pos:
            value = abs(ghost_pos[1] - new_pos[1])
            successors[f'Gore {value}'] = tuple(new_pos)

        # RIGHT
        new_pos=""
        for i in range(self.n):
            new_pos = ghost_right(ghost_pos, self.n, self.walls)
        if new_pos != ghost_pos:
            value = abs(ghost_pos[0] - new_pos[0])
            successors[f'Desno {value}'] = tuple(new_pos)
        print(successors)
        return successors
