...
def snake_up(snake_pos,obstacles,green_food):
    sp_new=snake_pos[:]
    head = snake_pos[0]
    if head[1]+1 <= 9 and (head[0],head[1]+1) not in obstacles and (head[0],head[1]+1) not in snake_pos:
        sp_new.insert(0,(head[0], head[1]+1))
        if (head[0],head[1]+1) not in green_food:
            sp_new=sp_new[:-1]
    return sp_new


def snake_down(snake_pos, obstacles,green_food):
    sp_new = snake_pos[:]
    head = snake_pos[0]
    if head[1]-1 >= 0 and (head[0], head[1]-1) not in obstacles and (head[0], head[1]-1) not in snake_pos:
        sp_new.insert(0, (head[0], head[1] - 1))
        if (head[0], head[1] - 1) not in green_food:
            sp_new = sp_new[:-1]
    return sp_new
...
def snake_left(snake_pos,obstacles,green_food):
    sp_new = snake_pos[:]
    head = snake_pos[0]
    if head[0]-1 >= 0 and (head[0]-1, head[1]) not in obstacles and (head[0]-1, head[1]) not in snake_pos:
        sp_new.insert(0, (head[0]-1, head[1]))
        if (head[0]-1, head[1]) not in green_food:
            sp_new = sp_new[:-1]
    return sp_new


def snake_right(snake_pos,obstacles,green_food):
    sp_new = snake_pos[:]
    head = snake_pos[0]
    if head[0] + 1 <= 9 and (head[0] + 1, head[1]) not in obstacles and (head[0] + 1, head[1]) not in snake_pos:
        sp_new.insert(0, (head[0] + 1, head[1]))
        if (head[0] + 1, head[1]) not in green_food:
            sp_new = sp_new[:-1]
    return sp_new
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        :rtype: dict
        """
        # (tuple(snake_pos), tuple(snake_dir), tuple(green))
        successors = dict()
        snake_pos=list(state[0])
        snake_dir=state[1]
        green = list(state[2])
        red_p=list(self.red)
        if snake_dir=="east":
            # PRAVO
            snake_newpos = snake_right(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["ProdolzhiPravo"] = (tuple(snake_newpos), 'east', tuple([food for food in green if snake_newpos[0] != food]))

            # DESNO
            snake_newpos = snake_down(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiDesno"] = (tuple(snake_newpos), 'south', tuple([food for food in green if snake_newpos[0] != food]))
            # LEVO
            snake_newpos = snake_up(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiLevo"] = (tuple(snake_newpos), 'north', tuple([food for food in green if snake_newpos[0] != food]))


        if snake_dir=="west":
            # PRAVO
            snake_newpos = snake_left(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["ProdolzhiPravo"] = (tuple(snake_newpos), 'west', tuple([food for food in green if snake_newpos[0] != food]))

            # DESNO
            snake_newpos = snake_up(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiDesno"] = (tuple(snake_newpos), 'north', tuple([food for food in green if snake_newpos[0] != food]))
            # LEVO
            snake_newpos = snake_down(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiLevo"] = (tuple(snake_newpos), 'south', tuple([food for food in green if snake_newpos[0] != food]))

        if snake_dir=="north":
            # PRAVO
            snake_newpos = snake_up(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["ProdolzhiPravo"] = (tuple(snake_newpos), 'north', tuple([food for food in green if snake_newpos[0] != food]))

            # DESNO
            snake_newpos = snake_right(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiDesno"] = (tuple(snake_newpos), 'east', tuple([food for food in green if snake_newpos[0] != food]))
            # LEVO
            snake_newpos = snake_left(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiLevo"] = (tuple(snake_newpos), 'west', tuple([food for food in green if snake_newpos[0] != food]))

        if snake_dir=="south":
            #PRAVO
            snake_newpos=snake_down(snake_pos, red_p,green)
            if snake_pos!=snake_newpos:
                successors["ProdolzhiPravo"] = (tuple(snake_newpos), 'south', tuple([food for food in green if snake_newpos[0] != food]))

            # DESNO
            snake_newpos = snake_left(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiDesno"] = (tuple(snake_newpos), 'west', tuple([food for food in green if snake_newpos[0] !=food]))
            #LEVO
            snake_newpos = snake_right(snake_pos, red_p,green)
            if snake_pos != snake_newpos:
                successors["SvrtiLevo"] = (tuple(snake_newpos), 'east', tuple([food for food in green if snake_newpos[0] != food]))
        return successors
...
if __name__ == '__main__':
    ...
    for i in range(n):
        red=input().split(",")
        red1=(int(red[0]), int(red[1]))
        green.append(tuple(red1))
    m=int(input())
    red=[]
    for i in range(m):
        red2 = input().split(",")
        red1 = (int(red2[0]), int(red2[1]))
        red.append(tuple(red1))
    snake=Snake(tuple(red),(tuple(snake_pos),snake_dir, tuple(green)))
