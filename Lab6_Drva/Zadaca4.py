...
if __name__=="__main__":
    ...
    num_trees=int(input())
    method=input()
    ent=[float(el) for el in input().split(" ")]
    entry=[ent[i] for i in range(len(ent)) if i!=ind]
    data=list()

    for row in dataset:
        row2=[row[i] for i in range(len(row)) if i!=ind]
        data.append(row2)
    ...
    classifier=RandomForestClassifier(n_estimators=num_trees, criterion=method, random_state=0)
